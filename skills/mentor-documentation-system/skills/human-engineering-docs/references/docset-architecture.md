...
A docset is a small linked book whose chapters stay near the 100-line soft target. `000-index.md` is the map and comprehensive abstract. Every other file answers one main question and links back to the index.
...
Inside a rolling-wave-planning batch, the docset root is the batch directory's own `docs/`, holding one chapter per feature at `NNN-<slug>.md`. If the repository already has an explicit human-documentation root, use its `docs/` child. Never place these files inside agent plan, scratch, task-state, or generated-code directories.

Corrected 2026-09-16: this path previously read `human/<task-slug>/`. The rolling-wave-planning v2 layout renamed `human/` to `docs/`. Existing docsets under `human/` match their own layout and stay where they are.
...
Split early when a file approaches 90 lines (the target is soft, so split for a reason, not to satisfy a counter), contains more than one major execution path, or needs more than three substantial H2 sections. Do not cut a paragraph, code block, or causal explanation in half.
