...
| `code-map` | Whole-repo structural queries in place of blind file reads | `graphify` (installed per project, not present in `~/.agents/skills`) | none | **Only** when the code-map row in `02-adapters.md` reads `enabled: true` and `graphify-out/graph.json` exists. Then every exploration and implementer packet carries the line: *"query the code map first, open only cited files"* |
