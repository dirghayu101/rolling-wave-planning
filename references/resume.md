...
- **Insert that will execute NEXT** (the normal case, since inserts land at "now"): it takes slot `(highest item opened so far) + 1` and **every still-`pending` item shifts +1** — rename its `rollout/<n>-…` dir and any `working/<n>-…` file, retitle its issue `[<N>.<i>]`, and fix every reference in `00-plan.md` (ledger, STATE, and any decision that spells out the order — add a dated note there, never a silent rewrite).
