...
Five fresh-agent scenarios must pass before tagging `v2.0.0` (plan decision 8, § 19). This
...
1. The orchestrator (Fable, or whoever is cutting the release) launches a **fresh light-tier
   subagent** — no prior context, no memory of this repo work.
...
5. A release is tagged only when **all five** scenarios pass. A failing scenario is a bug in the
