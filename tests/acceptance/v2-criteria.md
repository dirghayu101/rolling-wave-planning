...
| 26 | The adapter config lives in the SSOT, lists available tools by category with instructions, a default is generated during pre-planning, and the developer swaps values | `templates/02-adapters.md`, `templates/adapters.default.md`, `references/adapters.md` detection and delta procedure | | open |
| 27 | graphify set up in the mashric repo, with an enable/disable switch | mashric branch `chore/graphify-code-map`: `.graphifyignore`, gitignored `graphify-out/`, `docs/features/adapters.default.md` code-map row `enabled: true` | | open |
